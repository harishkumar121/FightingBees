    ...
    socket.on('loggedInUser', function(users){
        ...
        for(var i = 0; i < users.length; i++){
            if(friend.indexOf(users[i].name) > -1){
                arr.push(users[i]);
                // https://footballkik.s3.amazonaws.com/'+users[i].img+'
                var userName = users[i].name.toLowerCase();
                var list = '<img src="http://via.placeholder.com/300" class="pull-left img-circle" style="width:50px; height:50px; margin-right:10px;" /><p>' +
